...
//	uart3_init(230400);
    ...
    //初始化与蜂鸣器连接的硬件接口
    Buzzer_Init();
    Buzzer=0; // Keep PA8 low: buzzer permanently disabled.
	...
	//CAN通信接口初始化
	//CAN 位速率 = 42 MHz / [(SJW + BS1 + BS2) * Prescaler]
	//1 + 10 + 3 = 14 个时间量子，分频为 3，因此仍为 1 Mbps。
	//采样点为 (1 + 10) / 14 = 78.6%，可提高线缆存在传播延迟时的稳定性。
    CAN1_Mode_Init(1,3,10,3,0);
