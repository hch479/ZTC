...
int chassis_tasks_start(void)
{
    ...
    /*
     * 先创建无电机输出的串口任务，最后才创建控制任务。
     * 即使第二次创建因内存不足失败，也不会留下一个可输出 PWM 的孤立任务。
     */
    serial_result = xTaskCreate(chassis_serial_task,
                             "chassis_serial",
                             CHASSIS_SERIAL_STACK_SIZE,
                             NULL,
                             CHASSIS_SERIAL_TASK_PRIO,
                             NULL);
    if (serial_result != pdPASS)
    {
        return 0;
    }
    control_result = xTaskCreate(chassis_control_task,
                                 ...
                                 NULL);
    return (control_result == pdPASS) && (serial_result == pdPASS);
}
...
void chassis_serial_task(void *argument)
{
    ...
    uint32_t last_imu_status_ms = 0U;
    ...
    for (;;)
    {
        vTaskDelayUntil(&last_wake_time, pdMS_TO_TICKS(SERIAL_POLL_PERIOD_MS));
        now_ms = c30d_port_get_time_ms();

        /* 一次唤醒把串口队列中已有报文全部读完，降低突发参数报文丢失概率。 */
        ...
        /*
         * Acceleration and gyroscope data use the same sequence number, so the
         * upper computer can reject a mismatched pair. Raw data is not sent
         * until the stationary start-up calibration has completed.
         */
        if ((now_ms - last_imu_ms) >= IMU_TELEMETRY_PERIOD_MS)
        {
            if (imu_get_snapshot(&imu_snapshot) && imu_snapshot.calibrated)
            {
                chassis_encode_imu_vector(CHASSIS_CAN_ID_IMU_ACCELERATION,
                                          ...
                                          &first);
                chassis_encode_imu_vector(CHASSIS_CAN_ID_IMU_GYROSCOPE,
                                          imu_snapshot.gyro.x,
                                          imu_snapshot.gyro.y,
                                          imu_snapshot.gyro.z,
                                          (uint8_t)imu_snapshot.sample_sequence,
        ...
        /* Calibration progress remains visible even before vector data starts. */
        if ((now_ms - last_imu_status_ms) >= IMU_STATUS_PERIOD_MS)
        {
            if (imu_get_snapshot(&imu_snapshot))
            {
                chassis_encode_imu_status(imu_snapshot.sensor_type,
                                          imu_snapshot.calibrated,
                                          imu_snapshot.calibration_samples,
                                          imu_snapshot.calibration_target,
                                          (uint8_t)imu_snapshot.sample_sequence,
                                          &first);
                send_frame(&first);
            }
            last_imu_status_ms = now_ms;
        }
