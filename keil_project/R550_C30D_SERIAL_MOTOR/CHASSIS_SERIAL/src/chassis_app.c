                              ...
                              uint8_t hardware_enable)
{
    ...
    //限制上位机命令 最大线速度 0.60 m/s 最大角速度 1.50 rad/s
    ...
    if (output_allowed != 0U)
    {
        if (app->params.chassis_type == (uint8_t)CHASSIS_TYPE_ACKERMANN)
        {
            ...
            {   
                //  计算舵角 δ = atan(wheelbase × w / v)       δ          舵角，rad
                // v          车体中心线速度，m/s
                // w          期望角速度，rad/s
       ...
       //直行同步修正，左右电机、轮胎直径和摩擦力不可能完全一致。即使两个速度环都接近目标，长距离也可能出现累计偏差
    ...
    /* 计算里程计，里程计使用经过低通滤波的实测轮速，而不是目标速度。 */
